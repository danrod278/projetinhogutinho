export class Post {
    ...
    criarHomePage(iddivmain, idInputComentar) {
        ...
        var deletarPost = document.createElement('button')
        deletarPost.textContent = "Deletar"
        deletarPost.addEventListener("click", () => this.deletarPost())
        cabecalho.appendChild(deletarPost)
        ...
        if (input.length > 0) {
            ...
            try {
                fetch('/comentar', {
                    ...
                    body: JSON.stringify({ comentario: input, _id: this._id })
                }).then(response => {
                    if (!response.ok) {
